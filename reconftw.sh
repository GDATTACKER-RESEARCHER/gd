...
sub_passive(){
	if [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]
		then
			...
			eval amass enum -active -d $domain -o amass.txt $DEBUG_STD
...
nuclei_check(){
	if [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]
		then
			...
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/technologies/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_technologies.txt;
			printf "${yellow}\n\n Running : Nuclei Tokens${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/exposed-tokens/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_tokens.txt;
			printf "${yellow}\n\n Running : Nuclei Exposures${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/exposures/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_exposures.txt;
			printf "${yellow}\n\n Running : Nuclei CVEs ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/cves/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_cves.txt;
			printf "${yellow}\n\n Running : Nuclei Default Creds ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/default-logins/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_default_creds.txt;
			printf "${yellow}\n\n Running : Nuclei SubTko ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/takeovers/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_subtko.txt;
			printf "${yellow}\n\n Running : Nuclei DNS ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/dns/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_dns.txt;
			printf "${yellow}\n\n Running : Nuclei Miscellaneous ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/miscellaneous/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_miscellaneous.txt;
			printf "${yellow}\n\n Running : Nuclei Panels ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/exposed-panels/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_panels.txt;
			printf "${yellow}\n\n Running : Nuclei Security Misconfiguration ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/misconfiguration/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_misconfigurations.txt;
			printf "${yellow}\n\n Running : Nuclei Vulnerabilites ${reset}\n\n"
			cat ${domain}_probed.txt | nuclei -silent -t ~/nuclei-templates/vulnerabilities/ -H 'User-Agent: Mozilla/5.0 (Android 11; Mobile; rv:68.0) Gecko/68.0 Firefox/85.0' -o ${domain}_nuclei_vulnerabilities.txt && touch $called_fn_dir/.${FUNCNAME[0]};
			printf "\n\n"
			end=`date +%s`
			getElapsedTime $start $end
	...
	if [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ]
		then
			...
			for sub in $(cat ${domain}_probed.txt); do
				...
				ffuf -mc 200 -fc 404 -ac -sf -s -H "User-Agent: Mozilla/5.0 (X11; Linux x86_64; rv:72.0) Gecko/20100101 Firefox/72.0" -w $fuzz_wordlist -maxtime 900 -u $sub/FUZZ -or -o $dir/fuzzing/${sub_out}.tmp $DEBUG_STD
